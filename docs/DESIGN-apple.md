...
# DESIGN-apple — adaptasi Apple DESIGN.md untuk eksperimen SIGAP visual-fusion

- Sumber desain (dibaca penuh, disalin ke berkas ini): `/Users/nebukadnezar/Documents/Coding Claude/Pustaka/awesome-design-md/design-md/apple/DESIGN.md`
- Repo: `sigap-eksperimen`, branch: `eksperimen/visual-fusion` (Next.js 16 + Tailwind v4)
- Status berkas ini: salinan/adaptasi sumber Apple di atas. Frontmatter dan isi Apple dipertahankan apa adanya di bawah, kecuali bagian FUSI ini yang MENGIKAT eksperimen.
- Implementasi token: `src/app/globals.css`, blok `@theme` berprefix `ap-` yang ADDITIVE (token lama SIGAP tidak diubah).

## FUSI: keputusan yang mengikat eksperimen ini

Keputusan di bawah ini mengalahkan isi Apple di mana pun keduanya bertentangan. Berlaku hanya di permukaan eksperimen (`eksperimen/visual-fusion`), bukan di luarnya.

- (a) Display font TETAP Fraunces serif milik SIGAP, bukan SF Pro. Seluruh token `typography.*` Apple yang menyebut `SF Pro Display` / `SF Pro Text` dibaca ulang dengan substitusi display: `var(--font-fraunces), Georgia, serif` (token Tailwind SIGAP yang sudah ada: `font-display` / `font-serif`). Ukuran, bobot, line-height, dan letter-spacing Apple tetap dipakai (hero 56px/600/1.07/-0.28px, tile headline 40px/600/1.10, body 17px/400/1.47). Body/UI boleh memakai font sans SIGAP yang sudah ada (`font-sans`), bukan SF Pro Text.
- (b) Aksen interaktif Action Blue `#0066cc`. Semua link, pill CTA, dan sinyal fokus di permukaan eksperimen memakai Action Blue; varian fokus `#0071e3` untuk `:focus-visible` (outline 2px solid); varian terang `#2997ff` hanya untuk link di atas permukaan gelap. Aksen ini menggantikan aksen lain (mis. hijau daun SIGAP) hanya di permukaan eksperimen. Token: `ap-blue`, `ap-blue-focus`, `ap-sky` di `globals.css`.
- (c) Tile rhythm light / parchment / dark. Section full-bleed bergantian: putih (canvas) ↔ parchment `#f5f5f7` ↔ tile gelap `#272729` / `#2a2a2c`; tanpa gap antar tile (pergantian warna adalah divider, radius 0). Token: `ap-parchment`, `ap-pearl` (capsule terang `#fafafc`), `ap-tile1`, `ap-tile2`.
- (d) Rounded pill untuk CTA. CTA primer memakai `rounded.pill` (9999px); padding acuan 11px 22px (hero 14px 28px). Radius kecil (`sm` 8px) hanya untuk aksi utilitas kompak; kartu utilitas memakai `lg` 18px. Tile full-bleed tetap `none` (0px).
- (e) Tanpa emoji. Tanpa gradient dekoratif kecuali hero ShaderGradient. Aturan Apple "no decorative gradients" tetap berlaku; satu-satunya pengecualian yang diizinkan adalah hero ShaderGradient sebagai artefak eksperimen fusi (SIGAP x Apple x ShaderGradient x liquid glass). Fotografi/produk tetap menjadi pembawa atmosfer, bukan overlay gradient CSS.
- (f) Target sentuh 44px, `:focus-visible` selalu. Semua kontrol interaktif minimal 44x44px (mengikuti `button-icon-circular` 44px, `search-input` height 44px, `global-nav` height 44px). Setiap elemen fokus keyboard wajib menampilkan `:focus-visible` (outline 2px solid `#0071e3`, offset 2px). Aturan `:focus-visible` global yang sudah ada di `globals.css` tidak boleh dihapus; eksperimen hanya menambah/menimpa warna fokus ke token Apple di permukaannya.

Pemetaan token eksperimen (`@theme`, prefix `ap-`, di `src/app/globals.css`):

| Token | Nilai | Padanan Apple |
|---|---|---|
| `ap-blue` | `#0066cc` | `colors.primary` (Action Blue) |
| `ap-blue-focus` | `#0071e3` | `colors.primary-focus` |
| `ap-sky` | `#2997ff` | `colors.primary-on-dark` |
| `ap-ink` | `#1d1d1f` | `colors.ink` |
| `ap-parchment` | `#f5f5f7` | `colors.canvas-parchment` |
| `ap-pearl` | `#fafafc` | `colors.surface-pearl` |
| `ap-tile1` | `#272729` | `colors.surface-tile-1` |
| `ap-tile2` | `#2a2a2c` | `colors.surface-tile-2` |
| `ap-hairline` | `#e0e0e0` | `colors.hairline` |
| `ap-shadow` | `rgba(0, 0, 0, 0.22) 3px 5px 30px` | satu-satunya shadow produk |
...
- SF Pro Display + SF Pro Text — negative letter-spacing at display sizes for the signature "Apple tight" headline feel. (Di eksperimen ini dibaca sebagai Fraunces serif SIGAP — lihat FUSI (a).)
...
### Brand Gradient
**No decorative gradients.** Atmospheric depth on product photography (the iPhone 17 Pro camera plate, the Apple Watch bands, AirPods reflections) is inherent to the imagery, not a CSS gradient overlay. The environment page's hero uses photographic atmosphere (mountain vista at dawn) but no gradient tokens are defined. Apple is the rare luxury-brand site with zero gradient-based design tokens. (Pengecualian eksperimen: hero ShaderGradient — lihat FUSI (e).)

## Typography

### Font Family
- **Display**: `SF Pro Display, system-ui, -apple-system, sans-serif` — Apple's proprietary display face, optimized for sizes ≥ 19px. Defines the voice of every headline. (Eksperimen: diganti Fraunces serif SIGAP — lihat FUSI (a).)
- **Body / UI**: `SF Pro Text, system-ui, -apple-system, sans-serif` — the text-optimized variant used for body copy, captions, buttons, and links below 20px. (Eksperimen: display tetap Fraunces; body/UI memakai font sans SIGAP — lihat FUSI (a).)
...
- Eksperimen ini tidak memakai substitusi Inter: display memakai Fraunces serif SIGAP (lihat FUSI (a)).
...
- Set headlines in `{typography.hero-display}` or `{typography.display-lg}` with negative letter-spacing (`-0.28 → -0.374px`) to get the signature "Apple tight" cadence. (Eksperimen: jenis huruf display tetap Fraunces — lihat FUSI (a).)
...
- Don't use gradients as decorative backgrounds; atmosphere comes from photography. (Pengecualian eksperimen: hero ShaderGradient — lihat FUSI (e).)
...
- Global nav utility links are smaller (~32 × 80px) — they deliberately sit at a tighter target because they're precision desktop actions, and the mobile hamburger replaces them at ≤ 833px. (Eksperimen: aturan FUSI (f) menang — semua kontrol di permukaan eksperimen minimal 44px.)
- Eksperimen: setiap elemen fokus keyboard wajib `:focus-visible` (lihat FUSI (f)).
...
5. Display headlines stay SF Pro Display 600 with negative letter-spacing. Body stays SF Pro Text 400 at 17px. The boundary is unbreakable. (Eksperimen: display dibaca sebagai Fraunces 600 SIGAP — lihat FUSI (a).)
